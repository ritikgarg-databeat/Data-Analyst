...
import { useDeletePythonHistoryEntry, usePythonHistory } from "@/features/python/use-python-history";
...
interface PythonHistoryPanelProps {
  onSelect: (code: string) => void;
  workspaceId?: string;
  className?: string;
}

/** Recent Python Lab executions — click a row to reload that code, or delete it. Modeled on sql-lab/query-history-panel.tsx. */
export function PythonHistoryPanel({ onSelect, workspaceId, className }: PythonHistoryPanelProps) {
  const historyQuery = usePythonHistory({ workspaceId, limit: 50 });
  const deleteEntry = useDeletePythonHistoryEntry();
  ...
  if (historyQuery.isError) {
    return (
      <ErrorState
        ...
        message="We couldn't reach the API to load your Python execution history."
  ...
  if (items.length === 0) {
    return (
      <EmptyState
        ...
        description="Run some code and it'll show up here so you can revisit it later."
        className={className}
      />
    );
  }
      ...
      {items.map((item) => (
        <li key={item.id} className="group flex items-start gap-2 px-1 py-2">
          ...
          >
            ...
            <p className="mt-1 truncate font-mono text-xs text-foreground">{item.code.trim().split("\n")[0]}</p>
          </button>
          <Button
            type="button"
            size="icon"
